...
export function SpaceHero() {
  ...
  return (
    <section className="relative isolate min-h-screen overflow-hidden bg-black pt-28 text-white">
        ...
        {stars.map((star) => {
          ...
          return (
            <motion.button
              ...
              animate={
                isActive
                  ? { scale: shouldReduceMotion ? 1.15 : [1, 1.8, 1.25] }
                  : { opacity: shouldReduceMotion ? 0.55 : [0.25, 0.8, 0.25] }
              }
              transition={
                isActive ? { duration: 0.45 } : { duration: 3.2, repeat: Infinity, delay: star.id * 0.45 }
              }
            >
              <span
                className={`h-1.5 w-1.5 rounded-full transition-all duration-500 ${
                  isActive
                    ? 'bg-white shadow-[0_0_12px_4px_rgba(255,255,255,0.9),0_0_38px_14px_rgba(59,130,246,0.9)]'
                    : 'bg-slate-300 shadow-[0_0_8px_rgba(255,255,255,0.55)]'
                }`}
              />
            </motion.button>
          );
      ...
      <div className="relative z-20 mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center px-4 text-center sm:px-6">
        <motion.div
        ...
        >
          ...
          <a
            href="#services"
            className="mx-auto mt-10 grid h-11 w-11 place-items-center rounded-full border border-white/15 text-slate-300 transition hover:border-brand-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-brand-200"
